#!/system/bin/sh
# =========================================================================
# CAMPAIGN RATIOS NPU x GPU — VARIANTE MoE (Marco-Nano Q4_0, PAS de MTP)
# Teste : HTP0 seul · GPUOpenCL seul · ordres HTP,GPU et GPU,HTP
#         × ratios -ts 80/20 · 50/50 · 20/80 — CPU en auto (fallback)
# Ordre randomisé seed 20260903, cooldown <45C entre runs, protocole gardé
# Sortie : /data/local/tmp/camp_ratios_moe/{label}/ + synthese
# =========================================================================
RUNTIME=/data/local/tmp/npu
BIN=$RUNTIME/llama-server
MODEL=/data/local/tmp/Marco-Nano-Instruct.Q4_0.gguf
OUT=/data/local/tmp/camp_ratios_moe
PORT=8580
...
echo "START $(date)"
echo "CONFIG='Marco-Nano MoE Q4_0 + --fit off + -ngl 99 -t 8 -c 2048 n_predict=$NPRED CPU=auto (PAS de MTP)'" > "$OUT/synthese.txt"

# Ordre randomisé (seed 20260903)
